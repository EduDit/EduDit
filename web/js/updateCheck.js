...
// local server (serve.py / EduDitWeb.exe — not the read-only GitHub Pages
...
const DISMISSED_KEY = "edudit_dismissed_update";
...
// Only the app's own local server (serve.py / EduDitWeb.exe) can write
...
export function showUpdateBanner(info) {
  ...
  const banner = el("div", { class: "update-banner" }, [
    el("span", { class: "small", text: `EduDit ${info.version} is available.` }),
    status,
    updateBtn,
    dismiss,
  ]);
