...
test("parseImportedProfile rejects a backup missing a profile name", () => {
  let threw = false;
  try {
    parseImportedProfile(JSON.stringify({ edudit_profile_export: 1, profile: defaultProfile() }));
  } catch (e) {
    threw = true;
  }
...
test("parseImportedProfile rejects a backup missing profile data", () => {
  let threw = false;
  try {
    parseImportedProfile(JSON.stringify({ edudit_profile_export: 1, name: "Alice" }));
  } catch (e) {
    threw = true;
  }
