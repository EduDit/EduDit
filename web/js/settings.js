...
export class Settings {
    ...
    frame.appendChild(
      el("p", {
        ...
        text:
          ...
          "USB/keyboard interface or an audio/keyer interface to connect to EduDit.",
      })
    );
    return frame;
  }
  ...
  // double paddle can reach EduDit over either connection, but audio can
    ...
    if (audioSelected) {
      ...
      frame.appendChild(
        el("p", {
          class: "small muted",
          text:
            "Manual paddle input — each paddle press sends one dit or dah directly. EduDit " +
            "does not yet emulate an electronic keyer's auto-repeat/squeeze (iambic) behavior; " +
            "if your hardware performs iambic keying itself, EduDit will follow whatever it sends.",
        })
      );
    }
    return frame;
    ...
    if (s.keyConnection === "audio") {
      frame.appendChild(
        el("p", {
          class: "small muted",
          text:
            "Your keyer/interface already shapes dits and dahs in the tone itself — EduDit " +
      ...
      if (leftMapped) {
        frame.appendChild(
          el("p", {
            class: "small muted",
            text:
              "Your Morse key behaves like a USB mouse and sends Left Click. EduDit can use that click " +
  ...
  _keyDebounceRow() {
    ...
    wrap.appendChild(
      el("p", {
        class: "small muted",
        text:
          "How long a contact transition must hold before EduDit treats it as real keying " +
  ...
  _keyAudioDeviceSection() {
    ...
    frame.appendChild(
      el("p", { class: "small muted", text: "Which microphone/audio input EduDit listens to for your key." })
    );
  ...
  _keyAudioAdvancedContent() {
    ...
    wrap.appendChild(
      el("p", {
        class: "small muted",
        text:
          "How large a level change EduDit treats as a real key press versus background noise. " +
    ...
    wrap.appendChild(
      el("p", {
        class: "small muted",
        text:
          "Minimum tone/silence duration before EduDit accepts it as real keying rather than noise. " +
  ...
  _exportProfile() {
    ...
    const link = el("a", { href: url, download: `edudit-${name}.json` });
