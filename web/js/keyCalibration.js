...
export class KeyCalibration {
  ...
  _renderAudioIntro() {
    this._renderMount(
      el("div", { class: "slider-frame" }, [
        el("p", {
          text:
            "This measures your key's actual electrical signal so EduDit can tell key-down from " +
    ...
    if (Math.abs(diff) < 0.01 && relativeChange < 0.15) {
      this._renderMount(
        el("div", { class: "slider-frame" }, [
          ...
          el("p", {
            text:
              "EduDit couldn't find a reliable difference between your key untouched and pressed. " +
  ...
  _describeAudioError(err) {
    ...
    return "EduDit couldn't start audio input for calibration.";
  }
  ...
  _renderHidIntro() {
    ...
    this._renderMount(
      el("div", { class: "slider-frame" }, [
        el("p", {
          text: "This checks your key's contact for bounce (rapid, unintended extra transitions) so EduDit can filter it correctly.",
        }),
