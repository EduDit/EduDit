  ...
  async _handleMorseInputError(err) {
    if (err && err.code === "NEEDS_CALIBRATION") {
      const goCalibrate = await confirmDialog(
        "Audio key input hasn't been calibrated yet, so EduDit doesn't know which direction means " +
    ...
    if (err && (err.name === "NotAllowedError" || err.name === "SecurityError")) {
      await alertDialog(
        "EduDit couldn't access the microphone — permission was denied. Allow microphone access in " +
    ...
    await alertDialog(
      "EduDit couldn't start audio key input. You can still use the keyboard, or switch to a " +
