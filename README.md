...
[`https://edudit.github.io/EduDit/`](https://edudit.github.io/EduDit/).
...
**macOS app** — download `EduDitWeb.dmg` from a GitHub Release, open it,
and launch `EduDitWeb.app`. The app runs the full web edition locally and
...
This creates `dist\EduDit.exe` via PyInstaller — the tkinter desktop app,
...
This creates `dist\EduDitWeb.exe`, which bundles the web app and a Python
...
also creates a `EduDitWeb_app` folder next to the `.exe` — a writable copy
...
This creates `dist/EduDitWeb.app` and an installable
`dist/EduDitWeb.dmg`. Updated web assets are stored in
`~/Library/Application Support/EduDit/web`, where a normal Mac app has write
...
build_exe.bat                  build dist\EduDit.exe (desktop app)
build_web_exe.bat              build dist\EduDitWeb.exe (web app, bundled Python)
